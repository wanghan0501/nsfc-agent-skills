---
name: nsfc-figure
description: NSFC申请书图表制作指南（代码绘图）。用Mermaid/matplotlib制作技术路线图、研究内容关系图、关键科学问题逻辑图、甘特图等逻辑性强、文字标签精确的图表。视觉插画式的概念渲染图、机理插画、材料/器件结构图请改用 nsfc-ai-figure（AI文生图）。
version: 0.1.0
---
...
> 💡 **可选：AI 文生图绘制插画式图表**。本技能专注**代码绘图**（Mermaid/matplotlib），擅长流程图、关系图、甘特图等逻辑性强、文字标签精确的图。若需要**概念示意图、机理插画、材料/器件结构图、应用场景效果图**等以视觉表达为主、文字标签少的科研插图，可改用配套的 `nsfc-ai-figure` 技能（通过文生图 API 生成）。二者分工：**逻辑结构用代码图，视觉表现用 AI 图**，风格统一后并存。
