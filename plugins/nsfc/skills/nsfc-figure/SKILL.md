...
> 💡 **可选：AI 文生图绘制示意性插画**。本技能专注**代码绘图**（Mermaid/matplotlib），擅长流程图、关系图、甘特图等逻辑性强、文字标签精确的图，以及承载真实数据的研究基础图。若仅需**开篇领域/应用场景的概念插画**（设背景、抢第一印象、不承载精确科学结论），可改用配套的 `nsfc-ai-figure`（文生图，**可选、且须按规范声明 AI 使用**）。**注意**：精确的机理图、材料/器件结构图、研究基础结果图**不要交给 AI**（易画错、承载科学结论、合规存疑）——这些仍用本技能代码绘图或真实数据/手绘。分工：**逻辑与真实结果用代码图，仅背景视觉用 AI 图**，风格统一后并存。
